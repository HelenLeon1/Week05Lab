 ...
 * ↓ YOUR CODE HERE ↓ */
    ...
    let myGreeting = new Greeting('Helen', 'MA');
    myGreeting.hello()
 ...
 * ↓ YOUR CODE HERE ↓ */
    ...
    console.log(`Invoking describe method for myBook instance: ${myBook.describe()}`);
    
    let yourBook = new Book('The Joy Luck Club', 'Amy Tan'); 
    console.log(`Invoking describe method for yourBook instance: ${yourBook.describe()}`); 
...
class Fruit {
  ...
  describe() {
    return console.log(`A ${this.name} is ${this.color} and has a ${this.taste} taste.`);
  }
}
...
class List {
  ...
  displayMovies() {
    ...
    for(let i = 0; i < this.movies.length; i++) {
      allMovies += ` ${this.movies[i].title} is directed by ${this.movies[i].director}.`;
    }
    return allMovies;
  }
}

let movie1 = new Movie("Jurrassic Park", "Steven Spielberg");
